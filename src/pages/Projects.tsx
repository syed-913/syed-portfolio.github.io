...
const Projects = () => {
  ...
  useEffect(() => { getPublicProjects().then(setProjects).catch(() => setProjects([])).finally(() => setLoading(false)); }, []);
          ...
          return (
            <Reveal key={key} delay={index * 0.05}>
              <article className="project-index-item">
                ...
                {expanded && project.output && <div className="project-detail" dangerouslySetInnerHTML={{ __html: project.output }} />}
              </article>
            </Reveal>
          );
        })}
