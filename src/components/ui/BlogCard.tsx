    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mt-auto">
                <button
                    onClick={handleShare}
                    className={`group/btn flex items-center gap-2 font-mono text-xs text-primary bg-[#111417] px-4 py-2 rounded border hover:bg-primary/10 transition-all duration-200 ${copied ? 'border-green-500 text-green-400' : 'border-white/10 hover:border-primary/50'}`}
                >
                    <span className={`group-hover/btn:text-white ${copied ? 'text-green-500' : 'text-gray-500'}`}>$</span>
                    {copied ? 'Echo "Copied!"' : 'sh share_link.sh'}
                </button>
