...
const empty: Partial<Experience> = { role:'',company:'',duration:'',startDate:'',endDate:'',description:[],techStack:[],visible:true,order:1 };
export const ExperienceManager=()=>{
  ...
  return <AdminSection title="Experience" description="Use start/end months when possible. The public profile calculates total professional experience from the timeline automatically, so it cannot become stale." action={<button className="admin-secondary" onClick={()=>{setEditing(null);setForm({...empty,order:items.length+1})}}><Plus size={15}/> New</button>}>
    <form onSubmit={save} className="admin-editor-card"><div className="admin-form-grid">
      <AdminField label="Role"><AdminInput required value={form.role??''} onChange={e=>setForm(p=>({...p,role:e.target.value}))}/></AdminField>
      <AdminField label="Company"><AdminInput required value={form.company??''} onChange={e=>setForm(p=>({...p,company:e.target.value}))}/></AdminField>
      <AdminField label="Start month"><AdminInput type="month" value={form.startDate??''} onChange={e=>setForm(p=>({...p,startDate:e.target.value}))}/></AdminField>
      <AdminField label="End month" hint="Leave empty for a current role."><AdminInput type="month" value={form.endDate??''} onChange={e=>setForm(p=>({...p,endDate:e.target.value}))}/></AdminField>
      <AdminField label="Display duration" hint="Optional legacy/custom wording. Dates above drive the automatic experience total."><AdminInput placeholder="e.g. Jun 2025 — Present" value={form.duration??''} onChange={e=>setForm(p=>({...p,duration:e.target.value}))}/></AdminField>
      <AdminField label="Order"><AdminInput type="number" value={form.order??1} onChange={e=>setForm(p=>({...p,order:Number(e.target.value)}))}/></AdminField>
    </div>
    ...
    <div className="admin-list">{items.map(item=><div className="admin-list-row" key={item.id}><div><strong>{item.role}</strong><span>{item.company} · {displayExperienceDuration(item)}</span></div><div className="admin-row-meta"><span>{item.visible?'Public':'Hidden'}</span><button onClick={()=>{setEditing(item.id!);setForm(item)}}><Edit3 size={16}/></button><button className="danger" onClick={()=>remove(item.id!)}><Trash2 size={16}/></button></div></div>)}</div>
  </AdminSection>;
};
